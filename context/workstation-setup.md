...
End-to-end guide for bootstrapping takt on a fresh Debian workstation. Every step is scripted — an agent can run this top-to-bottom.
...
Dependencies: `PyYAML>=6.0`, `textual>=1.0.0`, `pyzmq>=26.0`, `claude-code-sdk>=0.1.0`.
...
- Starts `libvirtd`
- Creates `targets` NAT network (10.101.0.0/20, gateway 10.101.0.1)
- Enables IP forwarding
- Generates SSH key (`~/.ssh/id_ed25519_targets`)
- Downloads Debian 12 cloud image
- Creates the `deb-01` template VM (4 vCPU, 4GB RAM, 40GB disk, static IP 10.101.0.20)
...
After running, log out and back in so `libvirt`/`kvm` group membership takes effect.

## 4. Provision the Debian VM

Installs build tooling, editor configs, and sets default shell to zsh.
...
- APT: zsh, neovim, build-essential, cmake, clang, clang-format, clang-tidy, python3, python3-pip, python3-venv, rsync, curl, wget, locales
...
VMs access source code via a Samba share of `~/dev`. This avoids file transfer — VMs build directly from the share.
...
The Windows provisioning script maps `W:` drive automatically. To do it manually:
```powershell
...
Root repos are local mirrors that agents clone from. They live at `~/dev/root/<repo>`.
...
The `path` field in `config/repos.yaml` determines the directory name under `~/dev/root/`. Some repos use a `.git` suffix in the path (e.g. `mpl-wgpu.git`) — clone to match:
```bash
...
Template VMs are read-only base images. Create clones for actual use:
...
Network: `targets` (NAT, 10.101.0.0/20). DHCP range: 10.101.8.1 — 10.101.15.254 (not used by targets; they use static IPs).
...
The home directory needs `o+x` permission so `libvirt-qemu` can traverse into `~/libvirt/images/`:
```bash
chmod o+x ~/
```
...
- `~/libvirt/images/` — create directory, set permissions: ```bash sudo mkdir -p ~/libvirt/images sudo chown libvirt-qemu:kvm ~/libvirt/images chmod o+x ~/ ```
